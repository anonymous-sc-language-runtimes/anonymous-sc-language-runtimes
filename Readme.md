# Artifact

Artifact for "Cache Side Channel Attacks on Language Runtimes"

## Hardware/Software Setup
- CPU: Intel Xeon(R) Silver 4390Y SP
- DVFS: `Off`
- CpuFreq: `2.4GHz`
- Hyperthreading: `Off`
- ASLR: `Off`
- Kernel Commandline Parameters: `isolcpus=2,4,6,8,10,12,14 cgroup_enable=memory systemd.unified_cgroup_hierarchy=false`

## Case Studies
1. [QuickJS — OpenPGP.js](./experiments/quickjs_rsa/Readme.md)
2. [QuickJS — jpeg-js](./experiments/quickjs_jpeg/Readme.md)
3. [CPython — Dictionaries](./experiments/cpython_dictionary/Readme.md)
4. [CPython — `pow`](./experiments/cpython_pow/Readme.md)
5. [V8 — Elliptic](./experiments/v8_ecdh/Readme.md)

## Download Instructions

```bash
git clone --recurse-submodules -j$(nproc) https://github.com/anonymous-sc-language-runtimes/anonymous-sc-language-runtimes.git
```

## Build Instructions

```bash
mkdir build
cd build
cmake ..
make -j$(nproc)
```
